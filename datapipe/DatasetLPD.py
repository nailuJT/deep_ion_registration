...
class DatasetPrimalDual(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = np.load(path_CTmask)[:,sl-math.floor(nSliceBlock/2)-1:sl+math.floor(nSliceBlock/2),:]
        mask = mask.transpose(0,2,1)
        CTblock = np.load(path_CT)[:,sl-math.floor(nSliceBlock/2)-1:sl+math.floor(nSliceBlock/2),:]
        CTblock = CTblock.transpose(0,2,1)
        mask_image = np.load(path_CTmask_image)[:,sl-math.floor(nSliceBlock/2)-1:sl+math.floor(nSliceBlock/2),:]
        ...
        path = r'/project/med2/Ines.Butz/Data/ML/PrimalDual/system_matrices/straight/'+pat+'_1mm1mm3mm'
        ...
        for i in np.arange(len(Angles)):
            ...
            g_angles += [np.load(path+r'/proj_slice'+str(sl)+'_angle'+str(int(Angles[i]))+'.npy')]
            sysm_angles += [torch.load(path+'/sysm_slice'+str(sl)+'_angle'+str(int(Angles[i]))+'.pt')]
            sysm_angles_norm += [torch.load(path+'/sysm_slice'+str(sl)+'_angle'+str(int(Angles[i]))+'_norm.pt')]
        ...
        primal = torch.as_tensor(iCTinacc.copy()/norm_val, dtype = torch.float)
        
        g = torch.as_tensor(g_angles.copy()/norm_val, dtype = torch.float)
                
        gt = torch.as_tensor(iCTacc.copy()/norm_val, dtype = torch.float)
